...
class Mosaic(BaseMixTransform):
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        for labels in mosaic_labels:
            ...
            loc.append(labels["loc"])
            action.append(labels["action"])
        ...
        final_labels["loc"] = final_labels["loc"][good]
        final_labels["action"] = final_labels["action"][good]
...
class Format:
    ...
    def __call__(self, labels):
        ...
        labels["loc"] = torch.from_numpy(loc) if nl else torch.zeros(nl)
        labels["action"] = torch.from_numpy(action) if nl else torch.zeros(nl)
